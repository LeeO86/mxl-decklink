        ...
        std::vector<SettingMeta> buildSchema()
        {
            ...
            return {
                ...
                {"MXL_DECKLINK_CARD_ID", SettingKind::Global, "hex32", {}, "",
                    "BMDDeckLinkPersistentID of the card (hex, e.g. 0xa1b2c3d4). Stable across reboots; recommended selector. Optional — defaults to card index 0 when unset.", true},
                {"MXL_DECKLINK_CARD_NAME", SettingKind::Global, "string", {}, "",
                    "Card display name selector (lab use; not reboot-stable).", true},
                {"MXL_DECKLINK_CARD_INDEX", SettingKind::Global, "int", {}, "0",
                    "Zero-based card index selector. Default when no CARD_ID/NAME/INDEX is set (first-deploy friendly).", true},
                ...
                {"MXL_DOMAIN_PATH", SettingKind::Global, "path", {}, "/dev/shm/mxl",
                    "MXL domain directory (must be tmpfs-backed). Production examples use /Volumes/mxl/<domain> (CBC mxl-hands-on).", true},
                ...
                {"WEB_ENABLE", SettingKind::Global, "bool", {}, "true",
                    "Enable the web interface and the mutating REST API (§7.5). Health endpoints and /metrics are always served.", true},
                {"WEB_PORT", SettingKind::Global, "port", {}, "8080",
                    "Consolidated HTTP port: web UI, REST API, /livez /readyz /statusz, /metrics (§7.1).", true},
                {"MXL_CONFIG_FILE", SettingKind::Global, "path", {}, "", "JSON configuration file (§4.5). Env-only setting.", true},
                {"MXL_DOMAIN_SCAN_PATH", SettingKind::Global, "path", {}, "/dev/shm",
                    "Root directory scanned for MXL domains (§7.6). Prefer a dedicated tmpfs (e.g. /Volumes/mxl) over the host's whole /dev/shm.", true},
                {"MXL_HEALTH_MIN_HEALTHY_CHANNELS", SettingKind::Global, "int", {}, "1",
                    "Readiness threshold (§7.2). Auto-clamped to 0 when no channels are configured yet.", true},
